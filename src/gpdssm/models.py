...
from src.gpdssm.layers_ys import (HiddenTransitLayer as HTL_ys, RootTransitLayer as RTL_ys,
                                  ObservationLayer as OL_ys, HiddenNonTransitLayer as HNTL_ys)
...
class YuleSimonSingleModel(StandardSingleModel):
    ...
    def initialize_structure(self, retrain=False):
        ...
        constant_param = (self.num_class, self.M, self.J, self.warm_start, self.learning_rate)

        self.hidden_layers = [RTL_ys(self.dim_hidden[0], *constant_param)]
        self.hidden_layers += self.construct_hidden_layers(constant_param)
        self.observation_layer = OL_ys(self.dim_all[-1], *constant_param)
        self.layers = self.hidden_layers + [self.observation_layer]
    ...
    def initialize_structure_from_functions(self, hidden_layer_functions: list[RandomFeatureGP],
                                            observation_function: RandomFeatureGP):
        for i in range(self.num_hidden_layer):
            self.functions.append(deepcopy(hidden_layer_functions[i]))
            self.layers[i].function = self.functions[i]

        self.functions.append(deepcopy(observation_function))
        self.layers[-1].function = self.functions[-1]

    def construct_hidden_layers(self, constant_param):
        middle_layers = [HNTL_ys(self.dim_hidden[i], *constant_param)
                         for i in range(1, self.num_hidden_layer)]
        return middle_layers
